  ...
  return (
    <div className="animate-fade-in">
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
        <h1 style={{ fontSize: 22, fontWeight: 700, margin: 0 }}>{t("expenses")}</h1>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {filtered.map(r => {
            ...
            return (
              <div 
                key={r.id} 
                onClick={() => { setEditingItem({ ...r }); setSheetOpen(true); }} 
                style={{ 
                  background: "#1a1a1a", 
                  border: "1px solid #222", 
                  borderRadius: 16, 
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 12, color: "#888", marginTop: 4 }}>{rtl ? tag.nameAr : tag.name} · {r.status || "-"}</div>
                </div>
                <div style={{ textAlign: "end", flexShrink: 0 }}>
                  <div style={{ fontSize: 15, fontWeight: 700 }}>{fmt(r.amount, currency)}</div>
                  <div style={{ fontSize: 12, color: d <= 7 ? "#FF6B35" : "#888", marginTop: 2 }}>{r.endDate}</div>
                </div>
              </div>
            );
          })}
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14, padding: "4px 0" }}>
      ...
      <div style={{ display: "flex", gap: 12 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <input value={item.days} onChange={e => updateField("days", parseInt(e.target.value) || 0)} type="number" style={{ width: "100%", padding: 14, borderRadius: 14, border: "1px solid #333", background: "#0F0F0F", color: "#fff", fontSize: 15, boxSizing: "border-box" }} />
        </div>
      </div>
