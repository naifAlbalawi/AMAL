...
export function GanttStrip({ records, compact = false }) {
  ...
  const totalH = Math.max(records.length * 26 + 28, 50);
  ...
  return (
    <div ref={scrollRef} style={{ overflowX: "auto", overflowY: "hidden", width: "100%", border: "1px solid #222", borderRadius: 12, background: "#0F0F0F" }}>
      <svg width={totalW} height={totalH} style={{ display: "block" }}>
        {ticks.map((t, i) => (
          <text key={i} x={t.x} y={14} fill="#888" fontSize={10} textAnchor="middle">{t.label}</text>
        ))}
        {Array.from({ length: days }, (_, i) => {
          const x = labelW + i * dayW;
          return <line key={i} x1={x} y1={20} x2={x} y2={totalH} stroke="#1a1a1a" strokeWidth={1} />;
        })}
        {records.map((r, i) => {
          const y = 24 + i * 26;
          ...
          return (
            <g key={r.id}>
              <text x={labelW - 8} y={y + 16} fill="#ccc" fontSize={11} textAnchor="end">{r.name.length > (compact ? 10 : 14) ? r.name.slice(0, compact ? 9 : 13) + "…" : r.name}</text>
              <rect x={sx} y={y + 2} width={w} height={18} rx={6} fill={barColor} opacity={0.9} />
              {w > 35 && <text x={sx + w / 2} y={y + 15} fill="#fff" fontSize={9} textAnchor="middle">{(r.endDate || r.ends).slice(5)}</text>}
            </g>
          );
        })}
        {todayX >= labelW && todayX <= totalW && (
          <g>
            <line x1={todayX} y1={20} x2={todayX} y2={totalH} stroke="#fff" strokeWidth={1} strokeDasharray="4 2" />
            <text x={todayX} y={totalH - 4} fill="#fff" fontSize={9} textAnchor="middle">TODAY</text>
          </g>
        )}
      </svg>
    </div>
